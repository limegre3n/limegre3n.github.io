 ...
 *   7. pause → upload rejected 'paused', quota unchanged → unpause
...
check('event counter incremented', counter.data().photoCount === 1, `got ${counter.data().photoCount}`);
...
check('paused event rejects upload with no quota loss', res3.ok === false && res3.reason === 'paused', JSON.stringify(res3));
dev = (await getDoc(doc(db, 'devices', uid))).data();
check('quota unchanged after paused rejection', dev.snapsRemaining === 9);
